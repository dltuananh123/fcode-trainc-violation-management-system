...
static void makeAccount(Account *a, const char *id, const char *pass,
                        int role) {
...
static int writeFile(const char *path, const void *data, size_t itemSize,
                     int count) {
  ...
  if (count > 0) {
    fwrite(data, itemSize, (size_t)count, fp);
  }
  fclose(fp);
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  /* Hoc thuat (0) */
  makeMember(&members[mc], "SV0001", "Nguyen Van An", TEAM_ACADEMIC,
             MEMBER_ROLE_LEADER, 0, 0, 1);
  mc++;
  makeMember(&members[mc], "SV0002", "Tran Thi Bich", TEAM_ACADEMIC,
             MEMBER_ROLE_MEMBER, 20000, 2, 3);
  mc++;
  makeMember(&members[mc], "SV0003", "Le Hoang Cuong", TEAM_ACADEMIC,
             MEMBER_ROLE_MEMBER, 0, 0, 0);
  mc++;

  /* Ke hoach (1) */
  makeMember(&members[mc], "SV0004", "Pham Minh Duc", TEAM_PLANNING,
             MEMBER_ROLE_LEADER, 50000, 0, 2);
  mc++;
  makeMember(&members[mc], "SV0005", "Vo Thi Mai", TEAM_PLANNING,
             MEMBER_ROLE_MEMBER, 20000, 1, 1);
  mc++;
  makeMember(&members[mc], "SV0006", "Bui Quoc Phong", TEAM_PLANNING,
             MEMBER_ROLE_MEMBER, 0, 0, 1);
  mc++;

  /* Nhan su (2) */
  makeMember(&members[mc], "SV0007", "Do Thanh Giang", TEAM_HR,
             MEMBER_ROLE_LEADER, 0, 0, 0);
  mc++;
  makeMember(&members[mc], "SV0008", "Ngo Thi Hanh", TEAM_HR,
             MEMBER_ROLE_MEMBER, 20000, 3, 2);
  mc++;
  makeMember(&members[mc], "SV0009", "Ly Minh Kien", TEAM_HR,
             MEMBER_ROLE_MEMBER, 0, 0, 1);
  mc++;

  /* Truyen thong (3) */
  makeMember(&members[mc], "SV0010", "Ha Thanh Long", TEAM_MEDIA,
             MEMBER_ROLE_LEADER, 0, 0, 1);
  mc++;
  makeMember(&members[mc], "SV0011", "Dang Thi Ngoc", TEAM_MEDIA,
             MEMBER_ROLE_MEMBER, 40000, 4, 4);
  mc++;
  makeMember(&members[mc], "SV0012", "Cao Van Phu", TEAM_MEDIA,
             MEMBER_ROLE_MEMBER, 0, 0, 0);
  mc++;

  /* BCN members */
  makeMember(&members[mc], "BCN001", "Tran Quoc Bao", TEAM_ACADEMIC,
             MEMBER_ROLE_BCN, 0, 0, 0);
  mc++;
  makeMember(&members[mc], "BCN002", "Pham Thi Cuc", TEAM_PLANNING,
             MEMBER_ROLE_BCN, 0, 0, 0);
  mc++;
  ...
  makeViolation(&violations[vc++], "SV0002", dateSec(2026, 2, 5), REASON_ABSENT,
                20000, 1, PENALTY_FINE, "");
  ...
  makeViolation(&violations[vc++], "SV0004", dateSec(2026, 4, 1), REASON_ABSENT,
                50000, 0, PENALTY_FINE, "");
  ...
  /* SV0008 - 2 violations */
  makeViolation(&violations[vc++], "SV0008", dateSec(2026, 3, 5), REASON_ABSENT,
                20000, 1, PENALTY_FINE, "");
  ...
  makeViolation(&violations[vc++], "SV0011", dateSec(2026, 1, 8), REASON_ABSENT,
                20000, 1, PENALTY_FINE, "");
